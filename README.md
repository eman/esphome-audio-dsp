...
Built for finding and characterising persistent sounds — a machine whining
somewhere in the neighbourhood, a transformer hum, a pump that starts at 5 a.m.
— rather than for visualisers or voice assistants.
...
The source exists so the analyser and the stream can share one microphone. Use
either consumer on its own, or both.

## Why not build on ESPHome's `microphone`?

`i2s_audio` hardcodes `dma_desc_num = 4` and `dma_frame_num = 256` — 21 ms of
DMA buffering at 48 kHz. A consumer that blocks overruns it and the audio is
...
noisy band, because both average the whole band width — one 1.46 Hz tone in a
...
- **`peak_stability`** — the standard deviation of the per-frame peak
  ...
  while its level fades; noise does not. Measured on real sources: 0.2–1 Hz for
  a machine, 13–48 Hz for a lump in the noise floor. Two candidates that looked
...
```sh
ffmpeg -i http://node.local:8080/audio.wav -t 30 -c copy clip.wav   # analyse
...
the correct number and it is inaudible in every player — the audio sounds
...
exactly the analysis frame rate, 41 dB above the envelope floor in 4–12 kHz,
...
the dedicated audio PLL and divides to 3.072 MHz exactly for 48 kHz × 64.
...
The noise floor improved 2–3 dB as well.
